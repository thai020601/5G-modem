...
sierra_cell_info()
{
    ...
    if [ -n "$nr5g_nsa" ] ; then
        ...
        endc_lte_rsrp=$(echo "$response" | grep -e "PCC Rx0 RSRP:" -e "PCC RxM RSRP:" | awk '{print $8}')
        ...
        case $rat in
            "SA")
                ...
                nr_physical_cell_id=$(echo "$nr_info" | grep "NR5G Cell ID:" | awk '{print $4}')
                nr_band=$(echo "$nr_info" | grep "NR5G band:" | awk '{print $3}'| sed 's/n//')
                nr_bw=$(echo "$nr_info" | grep "NR5G dl bw:" | awk '{print $4}')
                nr_rsrp=$(echo "$nr_info" | grep "NR5G RSRP (dBm):" | awk '{print $4}')
                nr_rsrq=$(echo "$nr_info" | grep "NR5G RSRQ (dB):" | awk '{print $8}')
                nr_sinr=$(echo "$nr_info" | grep "NR5G SINR (dB):" | awk '{print $4}')
                nr_arfcn=$(echo "$nr_info" | grep "NR5G Rx chan:" | awk '{print $4}')
            ;;
            "LTE"|"CAT-M"|"CAT-NB")
                ...
                lte_cell_id=$(echo "$response" | grep "Cell ID:" | awk '{print $6}')
                lte_freq_band_ind=$(echo "$response" | grep "LTE band:" | awk '{ print $3}')
                #ul_bandwidth_num=$(echo "$response" | grep "LTE Tx chan:" | awk '{print $8}')
                lte_ul_bandwidth=$(sierra_get_bandwidth "LTE" $ul_bandwidth_num)
                #dl_bandwidth_num=$(echo "$response" | grep "LTE Rx chan:" | awk '{print $4}')
                ...
                lte_rsrp=$(echo "$response" | grep -e "PCC Rx0 RSRP:" -e "PCC RxM RSRP:" | awk '{print $8}')
                lte_rsrq=$(echo "$lte_info" | grep -A1 "Serving:" | tail -1 | awk '{print $11}')
                lte_sinr=$(echo "$lte_info" | grep -A1 "Serving:" | tail -1 | awk '{print $9}')
            ;;
            "WCDMA")
